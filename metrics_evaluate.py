...
from sdpa_evaluation import sdpa_evaluation
...
string.""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument('--evaluation_factors', '--ef', type=str, help='choose different evaluation factors', choices=['mota', 'mts', 'sda', 'sdpa'], required=True)
...
if __name__ == '__main__':
    ...
    if args.evaluation_factors == 'mota':
        ...
        print(mm.io.render_summary(summary, formatters=fmt, namemap=mm.io.motchallenge_metric_names))
        logging.info('Completed')
    elif args.evaluation_factors == 'sda':
    ...
    elif args.evaluation_factors == 'sdpa':
        track_result_path = os.path.join(args.track_results, 'tracks')
        gt_data_path = 'data/gts'
        default_yaml = 'config/septrackernet.yaml'
        default_cfg = open(default_yaml)
        ...
        sdpa_evaluation(cfg, track_result_path, gt_data_path)
