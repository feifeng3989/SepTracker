...
from septracker_modules import *
from tracking_toolbox import *
from torch_geometric.data import Data, DataLoader
import time

class SeptrackerNet(nn.Module):
    def __init__(self, cfg, logger=None, is_eval=False):
        super(SeptrackerNet, self).__init__()
        ...
        self.det_gat_layers = nn.ModuleList([HGATDualSpaceLite(cfg) for _ in range(cfg.GAT.NUM)])
        ...
        self.loss_gate = nn.Parameter(torch.tensor([1.0, 0.1]))
    
    def compute_total_loss(self, asso_loss, group_loss):
        w = F.softmax(self.loss_gate, dim=0)
        return w[0] * asso_loss + w[1] * group_loss
    ...
    def forward(self, tracks_all, detections, end_marks):
        ...
        for i, (predicted_feature, det_embedding, track_mark, det_mark) in enumerate(zip(predicted_features, det_embeddings, tracklet_marks, detection_marks)):
            ...
            hist_positions = [track[1].cpu() for track in tracks_all[i]][:track_mark]
            predictions = [track[0].cpu() for track in tracks_all[i]][:track_mark]
            ...
            asso_index = torch.nonzero(adjacent_matrix, as_tuple=False).t()
    ...
    def generate_graph_edge(self, positions):
        ...
        edge_index = torch.nonzero(track_adjacent_matrix, as_tuple=False).t()
        return edge_index
