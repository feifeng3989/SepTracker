...
from septracker_detector import *

class SeptrackerDataForVideo:
...
def generate_videos(video_size, cfg):
    ...
    for gt_idx, gt in enumerate(gts):
        ...
        data_sliced = SeptrackerDataForVideo(data_list)
        ...
        for frame_id in range(1, last_frame + 1):
            ...
            bg_dir = os.path.join(cfg.DATA.BG_ROOT, str(num_dir))
            bgs = sorted(os.listdir(bg_dir),key=lambda x: int(os.path.splitext(x)[0]))
            enhanced_bg_list = bg_list_enhance(bgs)
            bg_list_len = len(enhanced_bg_list)
            
            org_bg_figure = tifffile.imread(os.path.join(bg_dir, enhanced_bg_list[frame_id % bg_list_len]))

            bg_figure = copy.copy(org_bg_figure)

            if num_dir in [4, 7, 14, 16]:
                stretched_bg_figure = bg_stretch(bg_figure,top_extend=1400, bot_extend=300)
            else:
...
def main():
    ...
    parser.add_argument('--cfg', type=str, default='config/septrackernet.yaml', help='Name of the cfg.')
